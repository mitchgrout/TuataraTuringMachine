 ...
 * TM_Transition.java
 *
 * Created on November 7, 2006, 12:12 PM
 *
 */
...
public class TM_Transition implements Serializable
{
    ...
    public void paint(Graphics g, Collection<TM_Transition> selectedTransitions, TM_Simulator simulator)
    {
        ...
        {
            Point2D perpendicular1 = new Point2D.Float(-(m_controlPtY - (m_fromState.getY() + TM_State.STATE_RENDERING_WIDTH/2)), m_controlPtX - (m_fromState.getX() + TM_State.STATE_RENDERING_WIDTH/2));
            ...
            Point2D controlPoint1 = new Point2D.Float(m_controlPtX + (float)perpendicular1.getX(),  m_controlPtY + (float)perpendicular1.getY());
            Point2D controlPoint2 = new Point2D.Float(m_controlPtX - (float)perpendicular1.getX(),  m_controlPtY - (float)perpendicular1.getY());
            
            g2d.draw(new CubicCurve2D.Float(m_fromState.getX() + TM_State.STATE_RENDERING_WIDTH/2, m_fromState.getY() + TM_State.STATE_RENDERING_WIDTH/2, (int)controlPoint1.getX(), (int)controlPoint1.getY(), (int)controlPoint2.getX(), (int)controlPoint2.getY(), m_toState.getX() + TM_State.STATE_RENDERING_WIDTH/2, m_toState.getY() + TM_State.STATE_RENDERING_WIDTH/2));
        }
    ...
    public void paintArrowHead(Graphics g)
    {
        ...
        //Point2D endPoint = new Point2D.Float(m_toState.getX(), m_toState.getY());        
        ...
        //render it
        //g2d.drawLine((int)arrowLoc.getX(), (int)arrowLoc.getY(), (int)p1.getX(), (int)p1.getY());
        //g2d.drawLine((int)arrowLoc.getX(), (int)arrowLoc.getY(), (int)p2.getX(), (int)p2.getY());
        //final GradientPaint blueToWhite = new GradientPaint((int)arrowLoc.getX(),(int)arrowLoc.getY(),Color.BLUE,(int)(p1.getX() + p2.getX())/ 2, (int)(p1.getY() + p2.getY())/ 2,Color.WHITE);
    ...
    private Point2D getArrowTangentVector(Point2D arrowLoc)
    {
        ...
        else
        {
            ...
            curve.subdivide(curve, null);
            controlPoint =  curve.getCtrlP2();
            */
            Point2D.Double startOfCurve = new Point2D.Double(m_fromState.getX() + TM_State.STATE_RENDERING_WIDTH/2, m_fromState.getY() + TM_State.STATE_RENDERING_WIDTH/2);
    ...
    /** Returns true iff the specified panel coordinates are within the space taken up by the arrowhead
     *  in the centre of the this transition.   The size of the containing box around the arrow is the
     *  same as the size of the containing box around the action text in actionContainsPoint.
     */
    public boolean arrowContainsPoint(int x, int y, Graphics g)
    {
        ...
        AffineTransform trans = AffineTransform.getTranslateInstance(arrowLocation.getX() - metrics.stringWidth(actionString) / 2, arrowLocation.getY());
        Shape translated = trans.createTransformedShape(boundingBox);
    ...
    public static Point2D getControlPointGivenMidpoint(Point2D midpoint, TM_State fromState, TM_State toState)
    {
        ...
        else
        {
            ...
            //newCPX -= TM_State.STATE_RENDERING_WIDTH / 2;
            //newCPY -= TM_State.STATE_RENDERING_WIDTH / 2;
        }
    ...
    public Shape getInputSymbolBoundingBox(Graphics g)
    {
        ...
        boundingBox = new Rectangle2D.Double(boundingBox.getMinX(),
             boundingBox.getMinY(),
             /*boundingBox.getWidth()*/ symbolWidth  + 2 * SELECTED_SYMBOL_BOX_PAD_X,
    ...
    public Shape getOutputSymbolBoundingBox(Graphics g)
    {
        ...
        Shape translated = trans.createTransformedShape(boundingBox);
        //Rectangle2D boundingBox = new Rectangle2D.Float(m_controlPtX - 10, m_controlPtY - 10, 20, 20);
